...
if user_query := st.chat_input("Асуултаа энд бичнэ үү..."):
    ...
    # Нөхцөл 2: Тодорхой хугацаа зааж асуусан бол
        ...
        # ✅ ЗАГВАР ШИНЭЧЛЭЛ: Pandas болон `or`-ын алдааг бүрэн засав.
        delivered_only_df = filtered_df[filtered_df['мэдээ'].isin(['ХRef хүргэсэн', 'Хүргэсэн'])]
        
        if 'тоо ширхэг' in filtered_df.columns:
            total_items_delivered = delivered_only_df['тоо ширхэг'].sum()
        else:
            total_items_delivered = len(delivered_only_df)
